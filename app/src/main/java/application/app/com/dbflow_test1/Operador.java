package application.app.com.dbflow_test1;
...
import application.app.com.dbflow_test1.daoEntity.Datos;
import application.app.com.dbflow_test1.daoEntity.Datos_Table;
...
public class Operador {
    ...
    public Map<String,String> insertaAleatorio(Integer c){
        ...
        if (ins) {
            id = new Select(Datos_Table.id,Method.max(Datos_Table.id)).from(Datos.class).querySingle().getId();
        }
    ...
    public Map<String,String> consulta(){
        ...
        while ( dts == null ) {
            dts = SQLite.select().from(Datos.class).queryList();
            memApp = getMemoryApp();
            memDev = getMemoryDevice();
        }
    ...
    public Map< String, String > actualizaAleatorio(){
        ...
        Map<String,Double> memApp = null;
        Map<String,Double> memDev = null;
        long id = new Select(Datos_Table.id, Method.min(Datos_Table.id)).from(Datos.class).querySingle().getId();
        long n = getCant();
        boolean up = false;
     ...
     */
    public long getCant(){
        return new Select(Method.count(Datos_Table.id)).from(Datos.class).count();
    }
